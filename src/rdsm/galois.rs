...
pub(crate) const QR_CODEWORD_GEN: Element = 0x11D;
// qr format generator/divisor polynomial, 0b10100110111
pub(crate) const QR_FORMAT_GEN: Element = 0x537;

// an element in the finite field GF(2^8)
pub(crate) type Element = u32;
...
pub(crate) const BLANK_EXP_LOG_LUTS: ExpLogLUTs = ([0; EXPVALUES], [0; LOGVALUES]);
...
pub(crate) fn qr_find_fmt(fcode: u16) -> Option<u8> {
...
pub(crate) fn carryless_multiply(x: Element, y: Element) -> Element {
    let mut output = 0;
...
pub(crate) fn galois_multiply(x: Element, y: Element, primitive: Element) -> Element {
    if primitive > 0 {
...
#[inline]
pub(crate) fn bit_length(n: Element) -> u32 {
...
pub(crate) fn carryless_divide(dividend: Element, divisor: Element) -> Element {
...
pub(crate) fn generate_exp_log_tables(tables: &mut ExpLogLUTs) {
    let (exp, log) = tables;
    let mut x: Element = 1;
...
mod _old {
    ...
    // "gf_mul"
    pub(crate) fn table_multiply(x: Element, y: Element, tables: &ExpLogLUTs) -> Element {
        if x == 0 || y == 0 {
            return 0;
        }
    ...
    // "gf_div"
    pub(crate) fn table_divide(x: Element, y: Element, tables: &ExpLogLUTs) -> Element {
        if y == 0 {
            panic!()
        }
        if x == 0 {
    ...
    pub(crate) fn table_pow(x: Element, power: u32, tables: &ExpLogLUTs) -> Element {
        let (exp, log) = tables;
